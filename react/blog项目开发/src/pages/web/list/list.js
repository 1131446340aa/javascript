...
import { TagsOutlined, CalendarOutlined, FolderFilled, EyeOutlined } from '@ant-design/icons'
import './list.less'
                ...
                <List dataSource={this.state.data}
                    renderItem={(item, index) => (
                        <List.Item key={index} style={{ display: "block" }}
                            actions={[
                                <IconText icon={TagsOutlined} text={item.tag.map(v => (
                                    <Tag key={item + Math.random()} color={color[Math.floor(Math.random() * color.length)]}>
                                        {v}
                                    </Tag>
                                ))} key="list-vertical-star-o" />,
                                <IconText icon={FolderFilled} text={item.category.map(v => (
                                    <Tag key={item + Math.random()} color={color[Math.floor(Math.random() * color.length)]}>
                                        {v}
                                    </Tag>
                                ))} key="list-vertical-like-o" />,
                                <IconText icon={CalendarOutlined} text={item.createdAt} key="list-vertical-message" />,
                                <IconText icon={EyeOutlined} text={`${item.readedCount}次预览`} key="list-vertical-message" />,
                            ]}>
                            <List.Item.Meta
