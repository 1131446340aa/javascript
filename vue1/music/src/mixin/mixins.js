...
export const mixin = {
    ...
    methods: {
        ...
        lrc_Width() {
            ...
            setTimeout(() => {
                that.$refs.in[that.currentlrc].style.opacity = 0;

                that.$refs.lrc.forEach(item => {
                    that.lrcWidth.push(item.offsetWidth);
                });

                that.$refs.active_lrc.forEach((item, key) => {
                    item.style.width = that.lrcWidth[key] + "px";
                });
            }, 1000);
        },
        nextone() {
        ...
        playing() {
            ...
            this.isPlay();
            this.$refs.in[this.currentlrc].style.animationPlayState = this.isplay ? 'running' : 'paused'
        },
        suiji_and_danqu() {
            if (this.playrules === 1) {
                this.currentIndex = this.index;
